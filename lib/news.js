...
async function fetchLiveNews(apiKey) {
    ...
    const queries = ["bitcoin cryptocurrency market", "stock market earnings", "forex dollar inflation"];
    for (const query of queries) {
        ...
        if (items.length) return { items, live: true, fallback: false, message: "Live GNews feed active." };
    }
    return { items: demoNews, live: false, fallback: true, errorCode: "GNEWS_EMPTY", message: "GNews ne koi article return nahi kiya — demo fallback active." };
}
